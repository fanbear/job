...
class ControllerExtensionAdvancedoptionadvancedoption extends Controller {
	public function index() {
		...
		if($product_info){
			 if($this->config->get('advancedoption_model_status')!=2){	
				$json['model'] = $product_info['model'];
			 }else{
				 $json['model'] = '';
			 }
			foreach ($this->request->post['option'] as $product_option_id => $product_option_value_id){
				$option_data_query = $this->db->query("SELECT * FROM ".DB_PREFIX."product_option_value_data WHERE product_option_value_id = '".(int)$product_option_value_id."'");
				if(!empty($option_data_query->row['model'])){
				 if($this->config->get('advancedoption_model_status')==1 || $this->config->get('advancedoption_model_status')==2){	
				   $json['model'] .= $option_data_query->row['model'];
				 }
				}
				
				if($this->config->get('advancedoption_upc') && $this->config->get('advancedoption_upc_status')){
				$json['upc'] = (isset($option_data_query->row['upc']) ? $option_data_query->row['upc'] : '');
				}
				if($this->config->get('advancedoption_ean') && $this->config->get('advancedoption_ean_status')){
				$json['ean'] = (isset($option_data_query->row['ean']) ? $option_data_query->row['ean'] : '');
				}
				if($this->config->get('advancedoption_jan') && $this->config->get('advancedoption_jan_status')){
				$json['jan'] = (isset($option_data_query->row['jan']) ? $option_data_query->row['jan'] : '');
				}
			}
		}
		...
		if ($product_query->num_rows && ($this->request->post['quantity'] > 0)):
			...
			if($this->config->get('advancedoption_qty_status')){
